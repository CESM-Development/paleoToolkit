...
!-----------------------------------------------------------------------
     ...
     &             grid_imask(:)     ,  ! original imask
     &             grid_ikmt(:)         ! new kmt variable
...
!-----------------------------------------------------------------------
     ...
     &        nc_grdikmt_id,     ! netCDF grid kmt id
...
!-----------------------------------------------------------------------
      ...
      print *, "grid_file_in:", grid_file_in
      print *, "enter grid_topo_in:"
      read(*,'(a80)') grid_topo_in
      print *, "grid_topo_in:", grid_topo_in
      print *, "enter grid_file_out:"
      read(*,'(a80)') grid_file_out
      print *, "grid_file_out:", grid_file_out

      allocate(grid_imask(grid_size))
      allocate(grid_ikmt(grid_size))
...
!-----------------------------------------------------------------------
     ...
     &                          8, 'unitless')
      ...
      ncstat = nf_def_var (nc_grid_id, 'grid_ikmt', NF_INT,
     &                     1, nc_gridsize_id, nc_grdikmt_id)
      call netcdf_error_handler(ncstat)

      ncstat = nf_put_att_text (nc_grid_id, nc_grdikmt_id, 'units',
     &                          8, 'unitless')
      call netcdf_error_handler(ncstat)
...
!-----------------------------------------------------------------------
     ...
     &                           grid_center_lat)
      ...
      ncstat = nf_put_var_int(nc_grid_id, nc_grdikmt_id, grid_ikmt)
      call netcdf_error_handler(ncstat)
