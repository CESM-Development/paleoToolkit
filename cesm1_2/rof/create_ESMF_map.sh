...
# Currently supported on cheyenne only
# 
#===============================================================================
echo $0
date
...
usage() {
  ...
  echo ' --serial'
  echo '   For cheyenne batch jobs only! Load the serial ESMF tools rather'
  echo '   than the parallel tools (necessary for mapping grids with a single'
  echo '   point).'
  echo ' --machine (or -mach)'
  echo '   Name of the machine you are running on. Currently supports cheyenne only. Note that this script will determines the'
  ...
  echo '                 (Leave unset on cheyenne and the tool'
  echo '                 will be loaded from modules)'
  echo '  MPIEXEC ------ Name of mpirun executable'
  echo '                 (default is mpirun.lsf on cheyenne; if'
  echo '                 you run interactively on cheyenne, mpi is not used)'
  echo '**********************************************************'
}
